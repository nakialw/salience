...
metadata:
  short-description: Create serial task plans
---
...
For wave-based plans with explicit write scopes, dependencies, checkpoints, and parallel work items, use `$parallel-implementation` (Codex) or `/parallel-implementation` (Claude Code) instead of this skill.

For external second opinions on a completed plan, use `$ensemble-check` (Codex) or `/ensemble` (Claude Code) after the plan exists.
